...
require(doctor_candidate.get("status") == "candidate-after-adapter-dry-run", "command split readiness must mark doctor candidate after adapter dry-run")
...
require("physical split preflight" in " ".join(doctor_candidate.get("requiredPreSplitActions") or []), "doctor candidate must keep physical split preflight as pre-split action")
