...
for family_id in ("help", "doctor"):
    ...
    require("help" in joined, f"{family_id} candidate actions must reference the help split sequence")
    require("make command-help-doctor-split-preflight-check" in set(candidate.get("requiredGates") or []), f"{family_id} candidate gates must include help/doctor split preflight")
