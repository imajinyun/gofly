...
expected_active_batch = "GOFLY-P18"
...
require(actual_tasks == expected_p18_tasks, f"aiflowTasks must be ordered {expected_p18_tasks!r}; got {actual_tasks!r}")
...
require(actual_task_gates == expected_p18_task_gates, f"P18 task gates mismatch: {actual_task_gates!r}")
require("p17 completion" in tasks[0].get("title", "").lower(), "P18 round 01 title must document P17 completion handoff")
...
for expected_round, item in enumerate(tasks, start=1):
    ...
    if expected_status == "completed":
        require(item.get("commit") == "pending-current-commit", f"{task_id}: completed P17 task must record pending current commit")
        require(bool(item.get("verification")), f"{task_id}: completed P17 task must record verification")
    else:
        require("commit" not in item or item.get("commit") in ("", None), f"{task_id}: queued P17 task must not claim a completed commit")
        require("verification" not in item or item.get("verification") in ("", None), f"{task_id}: queued P17 task must not claim completed verification")
...
require(p18_submission.get("completedTasks") == expected_p18_tasks[:1], "P18 completedTasks must contain the P17 completion handoff")
require(p18_submission.get("pendingTasks") == expected_p18_tasks[1:], "P18 pendingTasks must match the remaining queue order")
