...
if p17_path.is_file():
    p17_manifest = json.loads(p17_path.read_text(encoding="utf-8"))
else:
    p17_manifest = {}
    missing.append("docs/reference/governance-p17-roadmap.json is missing")
if p18_path.is_file():
    p18_manifest = json.loads(p18_path.read_text(encoding="utf-8"))
else:
    p18_manifest = {}
    missing.append("docs/reference/governance-p18-roadmap.json is missing")
if p19_path.is_file():
    p19_manifest = json.loads(p19_path.read_text(encoding="utf-8"))
else:
    p19_manifest = {}
...
require(actual_p16_tasks == expected_p16_tasks, f"P16 roadmap task ids mismatch: {actual_p16_tasks!r}")
...
require(p16_submission.get("completedTasks") == expected_p16_tasks, "P16 completedTasks must contain all active batch tasks")
...
for task_id in expected_p16_tasks:
    require(
...
for expected_round, item in enumerate(p16_tasks, start=1):
    ...
    for field in ("id", "title", "objective", "deliverable", "acceptanceGates", "commitPolicy"):
        require(bool(item.get(field)), f"{task_id}: {field} is required")
    gates = item.get("acceptanceGates") or []
    require(gates[0] == expected_p16_task_gates[expected_round - 1], f"{task_id}: first acceptance gate mismatch")
    for gate in gates:
        require(gate_is_known(gate, targets), f"{task_id}: acceptanceGate is not known: {gate!r}")
    require("commit" in item.get("commitPolicy", "").lower(), f"{task_id}: commitPolicy must mention commit")
    require(item.get("commit") == "pending-current-commit", f"{task_id}: completed task must record pending current commit")
...
p17_tasks = p17_manifest.get("tasks") or []
actual_p17_tasks = [
    item.get("id")
    for item in p17_tasks
    if isinstance(item, dict)
]
require(actual_p17_tasks == expected_p17_tasks, f"P17 roadmap task ids mismatch: {actual_p17_tasks!r}")
p17_submission = p17_manifest.get("aiflowSubmission") or {}
require(p17_submission.get("status") == "submitted", "P17 aiflowSubmission.status must be submitted")
require(p17_submission.get("completedTasks") == expected_p17_tasks, "P17 completedTasks must contain all submitted tasks")
require(p17_submission.get("pendingTasks") == [], "P17 pendingTasks must be empty after P17 closeout")
p17_submission_commands = p17_submission.get("submissionCommands") or []
require(len(p17_submission_commands) == 3, "P17 submissionCommands must document all three aiflow submit calls")
for task_id in expected_p17_tasks:
    require(
        any(task_id in command and "aiflow submit" in command for command in p17_submission_commands),
        f"P17 submissionCommands missing {task_id}",
    )
require("aiflow status" in str(p17_submission.get("queueStatusCommand") or ""), "P17 queueStatusCommand must document aiflow status")
p17_handoff = p17_manifest.get("previousBatchHandoff") or {}
require(p17_handoff.get("completedBatch") == "GOFLY-P16", "P17 previousBatchHandoff.completedBatch must be GOFLY-P16")
require(p17_handoff.get("completedTaskCount") == 3, "P17 previousBatchHandoff.completedTaskCount must be 3")
p17_runtime_policy = str(p17_submission.get("runtimeStatePolicy") or "")
for path in (".aiflow", ".harness", ".tmp-test", ".trae", "coverage.out", "bench/current.txt", "bench/regression-report.json", "bench/summary.md", "bin/gofly", "docs/superpowers"):
    require(path in p17_runtime_policy, f"P17 runtimeStatePolicy missing {path!r}")
require("committed by the current agent or human" in str(p17_submission.get("safetyPolicy") or ""), "P17 safetyPolicy must document commit ownership")
for expected_round, item in enumerate(p17_tasks, start=1):
    if not isinstance(item, dict):
        missing.append(f"P17 roadmap task must be an object: {item!r}")
        continue
    task_id = item.get("id", "<missing>")
    require(item.get("round") == expected_round, f"{task_id}: round must be {expected_round}")
    expected_status = "completed"
    ...
    for field in ("id", "title", "objective", "deliverable", "acceptanceGates", "commitPolicy"):
        require(bool(item.get(field)), f"{task_id}: {field} is required")
    gates = item.get("acceptanceGates") or []
    require(gates[0] == expected_p17_task_gates[expected_round - 1], f"{task_id}: first acceptance gate mismatch")
    for gate in gates:
        require(gate_is_known(gate, targets), f"{task_id}: acceptanceGate is not known: {gate!r}")
    require("commit" in item.get("commitPolicy", "").lower(), f"{task_id}: commitPolicy must mention commit")
    if expected_status == "completed":
...
p18_tasks = p18_manifest.get("tasks") or []
actual_p18_tasks = [
    item.get("id")
    for item in p18_tasks
    if isinstance(item, dict)
]
...
for task_id in expected_p18_tasks:
    require(
        any(task_id in command and "aiflow submit" in command for command in p18_submission_commands),
        f"P18 submissionCommands missing {task_id}",
    )
require("aiflow status" in str(p18_submission.get("queueStatusCommand") or ""), "P18 queueStatusCommand must document aiflow status")
p18_handoff = p18_manifest.get("previousBatchHandoff") or {}
require(p18_handoff.get("completedBatch") == "GOFLY-P17", "P18 previousBatchHandoff.completedBatch must be GOFLY-P17")
require(p18_handoff.get("completedTaskCount") == 3, "P18 previousBatchHandoff.completedTaskCount must be 3")
p18_runtime_policy = str(p18_submission.get("runtimeStatePolicy") or "")
for path in (".aiflow", ".harness", ".tmp-test", ".trae", "coverage.out", "bench/current.txt", "bench/regression-report.json", "bench/summary.md", "bin/gofly", "docs/superpowers"):
    require(path in p18_runtime_policy, f"P18 runtimeStatePolicy missing {path!r}")
require("committed by the current agent or human" in str(p18_submission.get("safetyPolicy") or ""), "P18 safetyPolicy must document commit ownership")
for expected_round, item in enumerate(p18_tasks, start=1):
    if not isinstance(item, dict):
        missing.append(f"P18 roadmap task must be an object: {item!r}")
        continue
    task_id = item.get("id", "<missing>")
    require(item.get("round") == expected_round, f"{task_id}: round must be {expected_round}")
    expected_status = "completed"
    ...
    for field in ("id", "title", "objective", "deliverable", "acceptanceGates", "commitPolicy"):
        require(bool(item.get(field)), f"{task_id}: {field} is required")
    gates = item.get("acceptanceGates") or []
    require(gates[0] == expected_p18_task_gates[expected_round - 1], f"{task_id}: first acceptance gate mismatch")
    for gate in gates:
        require(gate_is_known(gate, targets), f"{task_id}: acceptanceGate is not known: {gate!r}")
    require("commit" in item.get("commitPolicy", "").lower(), f"{task_id}: commitPolicy must mention commit")
    if expected_status == "completed":
...
p19_tasks = p19_manifest.get("tasks") or []
actual_p19_tasks = [
    item.get("id")
    for item in p19_tasks
    if isinstance(item, dict)
]
require(actual_p19_tasks == expected_p19_tasks, f"P19 roadmap task ids mismatch: {actual_p19_tasks!r}")
...
require(
    p19_submission.get("completedTasks") == expected_p19_tasks,
    "P19 completedTasks must contain all active batch tasks",
)
require(
    p19_submission.get("pendingTasks") == [],
    "P19 pendingTasks must be empty after budget promotion blocker review",
)
...
for task_id in expected_p19_tasks:
    require(
        any(task_id in command and "aiflow submit" in command for command in p19_submission_commands),
        f"P19 submissionCommands missing {task_id}",
    )
require("aiflow status" in str(p19_submission.get("queueStatusCommand") or ""), "P19 queueStatusCommand must document aiflow status")
p19_handoff = p19_manifest.get("previousBatchHandoff") or {}
require(p19_handoff.get("completedBatch") == "GOFLY-P18", "P19 previousBatchHandoff.completedBatch must be GOFLY-P18")
require(p19_handoff.get("completedTaskCount") == 3, "P19 previousBatchHandoff.completedTaskCount must be 3")
p19_runtime_policy = str(p19_submission.get("runtimeStatePolicy") or "")
for path in (".aiflow", ".harness", ".tmp-test", ".trae", "coverage.out", "bench/current.txt", "bench/regression-report.json", "bench/summary.md", "bin/gofly", "docs/superpowers"):
    require(path in p19_runtime_policy, f"P19 runtimeStatePolicy missing {path!r}")
require("committed by the current agent or human" in str(p19_submission.get("safetyPolicy") or ""), "P19 safetyPolicy must document commit ownership")
for expected_round, item in enumerate(p19_tasks, start=1):
    if not isinstance(item, dict):
        missing.append(f"P19 roadmap task must be an object: {item!r}")
        continue
    task_id = item.get("id", "<missing>")
    require(item.get("round") == expected_round, f"{task_id}: round must be {expected_round}")
    expected_status = "completed"
    ...
    for field in ("id", "title", "objective", "deliverable", "acceptanceGates", "commitPolicy"):
        require(bool(item.get(field)), f"{task_id}: {field} is required")
    gates = item.get("acceptanceGates") or []
    require(gates[0] == expected_p19_task_gates[expected_round - 1], f"{task_id}: first acceptance gate mismatch")
    for gate in gates:
        require(gate_is_known(gate, targets), f"{task_id}: acceptanceGate is not known: {gate!r}")
    require("commit" in item.get("commitPolicy", "").lower(), f"{task_id}: commitPolicy must mention commit")
    if expected_status == "completed":
        ...
        require("commit" not in item or item.get("commit") in ("", None), f"{task_id}: submitted task must not claim a completed commit")
        require("verification" not in item or item.get("verification") in ("", None), f"{task_id}: submitted task must not claim completed verification")

p20_tasks = p20_manifest.get("tasks") or []
actual_p20_tasks = [
    item.get("id")
    for item in p20_tasks
    if isinstance(item, dict)
]
require(actual_p20_tasks == expected_p20_tasks, f"P20 roadmap task ids mismatch: {actual_p20_tasks!r}")
p20_submission = p20_manifest.get("aiflowSubmission") or {}
require(
    p20_submission.get("status") == "mirrored-after-aiflow-build-blocker",
    "P20 aiflowSubmission.status must document adjacent aiflow build fallback",
)
require(
    p20_submission.get("completedTasks") == expected_p20_tasks,
    "P20 completedTasks must contain all active batch tasks",
)
require(
    p20_submission.get("pendingTasks") == [],
    "P20 pendingTasks must be empty after model/cache template depth closeout",
)
p20_submission_commands = p20_submission.get("submissionCommands") or []
require(len(p20_submission_commands) == 3, "P20 submissionCommands must document all three aiflow submit calls")
for task_id in expected_p20_tasks:
    require(
        any(task_id in command and "aiflow submit" in command for command in p20_submission_commands),
        f"P20 submissionCommands missing {task_id}",
    )
require("aiflow status" in str(p20_submission.get("queueStatusCommand") or ""), "P20 queueStatusCommand must document aiflow status")
p20_handoff = p20_manifest.get("previousBatchHandoff") or {}
require(p20_handoff.get("completedBatch") == "GOFLY-P19", "P20 previousBatchHandoff.completedBatch must be GOFLY-P19")
require(p20_handoff.get("completedTaskCount") == 3, "P20 previousBatchHandoff.completedTaskCount must be 3")
p20_runtime_policy = str(p20_submission.get("runtimeStatePolicy") or "")
for path in (".aiflow", ".harness", ".tmp-test", ".trae", "coverage.out", "bench/current.txt", "bench/regression-report.json", "bench/summary.md", "bin/gofly", "docs/superpowers"):
    require(path in p20_runtime_policy, f"P20 runtimeStatePolicy missing {path!r}")
require("committed by the current agent or human" in str(p20_submission.get("safetyPolicy") or ""), "P20 safetyPolicy must document commit ownership")
for expected_round, item in enumerate(p20_tasks, start=1):
    if not isinstance(item, dict):
        missing.append(f"P20 roadmap task must be an object: {item!r}")
        continue
    task_id = item.get("id", "<missing>")
    require(item.get("round") == expected_round, f"{task_id}: round must be {expected_round}")
    require(item.get("status") == "completed", f"{task_id}: status must be completed")
    ...
    for field in ("id", "title", "objective", "deliverable", "acceptanceGates", "commitPolicy"):
        require(bool(item.get(field)), f"{task_id}: {field} is required")
    gates = item.get("acceptanceGates") or []
    require(gates[0] == expected_p20_task_gates[expected_round - 1], f"{task_id}: first acceptance gate mismatch")
    for gate in gates:
        require(gate_is_known(gate, targets), f"{task_id}: acceptanceGate is not known: {gate!r}")
    require("commit" in item.get("commitPolicy", "").lower(), f"{task_id}: commitPolicy must mention commit")
    require(item.get("commit") == "pending-current-commit", f"{task_id}: completed task must record pending current commit")
