...
func buildAIToolManifestCommands() []aiToolCommand {
	commands := []aiToolCommand{
		...
		manifestCommand("gateway aggregation validate", []string{"gateway aggregation diff"}, "Validate a candidate BFF aggregation contract against current gateway config or compare old/new OpenAPI x-gofly-aggregation contracts.", "gofly gateway aggregation validate --config <gateway.json> --route <route> --candidate <aggregation.json> [--openapi-base old.json --openapi-candidate new.json] [--format text|json] [--json]", map[string]aiInputProperty{"config": stringProperty("Gateway config JSON containing routes."), "route": stringProperty("Route name or route key to compare."), "candidate": stringProperty("Candidate aggregation JSON."), "openapi-base": stringProperty("Base OpenAPI JSON containing x-gofly-aggregation."), "openapi-candidate": stringProperty("Candidate OpenAPI JSON containing x-gofly-aggregation."), "format": enumStringProperty("Output format.", outputText, outputJSON), "json": boolProperty("Print JSON envelope.")}, []string{outputText, outputJSON}, []string{"reads gateway config and candidate aggregation files", "or reads base/candidate OpenAPI files"}, "read", true, false, []string{"gofly gateway aggregation validate --config etc/edge.json --route bff-home --candidate etc/edge-aggregation-candidate.json --json", "gofly gateway aggregation validate --openapi-base old-openapi.json --openapi-candidate new-openapi.json --route home --json"}),
