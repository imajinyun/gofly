...
func buildAIToolManifestCommands() []aiToolCommand {
	commands := []aiToolCommand{
		...
		manifestCommand("rpc protoc", nil, "Run standard protoc Go plugins or generate a gofly-native gRPC scaffold with zRPC-compatible semantics through --zrpc_out.", "gofly rpc protoc <service.proto>... [--zrpc_out <dir> | --go_out <dir> --go-grpc_out <dir>] [flags]", map[string]aiInputProperty{
			"src":                   stringProperty("Protobuf source file; multiple files may also be supplied positionally."),
			...
			"zrpc_out":              stringProperty("Generate a runnable gofly-native RPC scaffold with zRPC-compatible semantics in this directory."),
			"module":                stringProperty("Scaffold Go module; inferred from go_package when omitted."),
			"multiple":              boolProperty("Aggregate proto inputs and split scaffold server, logic, and client output by service."),
			"client":                boolProperty("Generate scaffold clients; defaults to true."),
			"name-from-filename":    boolProperty("Use the proto filename instead of its package for the scaffold application name."),
		...
		}, []string{outputText}, []string{"runs protoc as an external process", "--zrpc_out writes a runnable scaffold and preserves existing business logic files", "--zrpc_out aggregates multiple primary proto files only when --multiple is selected", "forwards non-gofly plugin argv only when --allow-external-plugin is set", "rejects flag-like plugin values, URL schemes, whitespace, control characters and shell metacharacters", "does not execute shell strings"}, "high", false, true, []string{"gofly rpc protoc greeter.proto --zrpc_out . --module example.com/greeter", "gofly rpc protoc catalog.proto inventory.proto --zrpc_out . --module example.com/platform --multiple", "gofly rpc protoc greeter.proto -I . --go_out . --go-grpc_out .", "gofly rpc protoc greeter.proto --plugin protoc-gen-openapi --allow-external-plugin --extra --openapi_out=."}),
