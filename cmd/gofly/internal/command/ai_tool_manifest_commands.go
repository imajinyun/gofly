...
func buildAIToolManifestCommands() []aiToolCommand {
	commands := []aiToolCommand{
		...
		manifestCommand("gateway aggregation validate", []string{"gateway aggregation diff"}, "Validate a candidate BFF aggregation contract against current gateway config and report additive or breaking step/shape changes.", "gofly gateway aggregation validate --config <gateway.json> --route <route> --candidate <aggregation.json> [--format text|json] [--json]", map[string]aiInputProperty{"config": stringProperty("Gateway config JSON containing routes."), "route": stringProperty("Route name or route key to compare."), "candidate": stringProperty("Candidate aggregation JSON."), "format": enumStringProperty("Output format.", outputText, outputJSON), "json": boolProperty("Print JSON envelope.")}, []string{outputText, outputJSON}, []string{"reads gateway config and candidate aggregation files"}, "read", true, false, []string{"gofly gateway aggregation validate --config etc/edge.json --route bff-home --candidate etc/edge-aggregation-candidate.json --json"}),
