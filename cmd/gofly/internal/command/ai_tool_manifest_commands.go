...
func buildAIToolManifestCommands() []aiToolCommand {
	commands := []aiToolCommand{
		...
		manifestCommand("api cleanup stale", []string{"api clean stale"}, "Read .gofly/stale-api-files.json and report stale go-zero-compatible API handler/logic files; deletes only when --execute is set.", "gofly api cleanup stale --dir <project> [--execute] [--json]", map[string]aiInputProperty{"dir": stringProperty("Project root containing .gofly/stale-api-files.json."), "execute": boolProperty("Delete validated stale .go files under internal/handler or internal/logic; omitted means report-only."), "json": boolProperty("Print JSON envelope.")}, []string{outputText, outputJSON}, []string{"reads .gofly/stale-api-files.json", "report-only by default", "--execute removes only validated stale .go files under internal/handler or internal/logic"}, "medium", true, true, []string{"gofly api cleanup stale --dir .", "gofly api cleanup stale --dir . --execute --json"}),
