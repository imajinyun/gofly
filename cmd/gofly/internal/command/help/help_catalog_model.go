...
func modelCommandHelp(command string) (Topic, bool) {
	...
	case "model mongo":
		return Topic{Name: "model mongo", Short: "Generate Mongo repository skeleton.", Usage: "gofly model mongo --type <name> --dir <dir> [--package <pkg>]", Flags: []string{"--type, -t <name>     model type name", "--dir, -d <dir>       output directory", "--package <pkg>       generated package name"}, Examples: []string{"gofly model mongo -t UserProfile -d internal/model"}}, true
	case "model":
		return Topic{
			Name:  "model",
