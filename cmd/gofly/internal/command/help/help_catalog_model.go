...
func modelCommandHelp(command string) (Topic, bool) {
	switch command {
	case "model mysql", "model pg":
		...
		return Topic{
			...
			Commands: []Command{{Name: "ddl", Short: "generate SQL model code from DDL"}, {Name: "datasource", Short: "generate model code by introspecting a datasource"}},
	...
	case "model mongo":
		return Topic{Name: "model mongo", Short: "Generate Mongo repository skeleton.", Usage: "gofly model mongo --type <name> --dir <dir> [--package <pkg>]", Flags: []string{"--type, -t <name>                             model type name", "--dir, -d <dir>                               output directory", "--package <pkg>                               generated package name", "--style go_zero|driver|go_zero_mongo          model style", "--prefix, -p <prefix>                         cache key namespace prefix"}, Examples: []string{"gofly model mongo -t UserProfile -d internal/model", "gofly model mongo -t UserProfile -d internal/model --cache --easy --style go_zero_mongo"}}, true
	case "model":
		return Topic{
			Name:  "model",
