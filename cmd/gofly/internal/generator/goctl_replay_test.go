...
func assertInventoryGoctlReplayArtifacts(t *testing.T, outDir string, fixture goctlReplayFixture) {
	...
	for _, want := range []string{
		...
		"item, ok, err := c.cache.Peek(ctx, id)",
		"key := redisIndexListCacheKey(version, indexListKeyByStatus(status, limit, offset))",
		"key := redisIndexListCacheKey(version, indexCountKeyByStatus(status))",
		"c.listVersionByStatus.Set(ctx, \"current\", redisIndexListVersionValue())",
