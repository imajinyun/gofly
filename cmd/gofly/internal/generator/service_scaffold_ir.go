...
func serviceScaffoldData(opts ServiceScaffoldOptions) map[string]string {
	return withGeneratedResilienceTemplateData(applyOperatorHistoryTemplateData(map[string]string{
		...
		"Port":             "8080",
		"RPCPort":          "8081",
		"RPCService":       lowerName(opts.Name) + ".Greeter",
		"Replicas":         "2",
...
func mergeServiceScaffoldExtras(files map[string]string, opts ServiceScaffoldOptions, style string, profile GenerationProfile) {
	...
	if strings.EqualFold(opts.Kind, "rpc") {
		if profile == ProfileGoZeroCompatible {
			files[opts.Name+".proto"] = goZeroRPCNewTemplate
		} else {
			files[opts.Name+".proto"] = rpcNewTemplate
		}
		if style == ServiceStyleProduction {
			if profile == ProfileGoZeroCompatible {
				files[filepath.Join("internal", "config", "config_test.go")] = goZeroRPCConfigTestTemplate
			} else {
