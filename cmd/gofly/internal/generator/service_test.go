...
func TestGenerateService(t *testing.T) {
	...
	for _, want := range []string{
		...
		`"generated.rpcMuxOperatorHistory"`,
		"rpcServer.MuxDiagnosisOperatorActionHistorySnapshot(5)",
...
func TestGenerateNewServiceVariantsBoundaries(t *testing.T) {
	...
	for _, want := range []string{
		...
		"rpc.NewRPCMuxDiagnosisOperatorHistoryFileStore(filepath.Join(root, relPath))",
