...
func TestGenerateService(t *testing.T) {
	...
	greeterData, err := os.ReadFile(filepath.Join(dir, "internal", "api", "grpc", "v1", "greeter", "greeter.go"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{
		...
		"rpcServer.MuxDiagnosisOperatorHistoryIntegritySnapshot(ctx)",
	...
	for _, want := range []string{
		...
		`runGeneratedControlPlaneSmoke(t, repo, restAddr, adminAddr)`,
		...
		`runGeneratedControlPlaneSmoke(t, repo, recommendedRestAddr, recommendedAdminAddr)`,
	...
	for _, want := range []string{
		...
		"fileSecretRoot",
		"mux operator history config is missing",
		"mux operator history store path is missing",
		"go run ./internal/config/production_check.go",
		"real mux subprocess profile must not be generated by default",
		"real mux operator history must not be generated by default",
		"kind: NetworkPolicy",
		"kind: HorizontalPodAutoscaler",
		"ServiceMonitor",
	...
	checkGoData, err := os.ReadFile(filepath.Join(dir, "internal", "config", "production_check.go"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
	...
	greeterClientTestData, err := os.ReadFile(filepath.Join(dir, "internal", "api", "grpc", "v1", "greeter", "greeter_client_test.go"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
...
func TestGenerateNewServiceVariantsBoundaries(t *testing.T) {
	...
	for _, want := range []string{
		...
		`"otelCompatible": {"enabled": false, "sink": "slog", "profile": "", "profileRef": "env://GOFLY_MUX_OTEL_PROFILE", "fileSecretRoot": "etc/secrets"`,
		`"operatorHistory": {"enabled": false, "store": "file://mux-operator-history.jsonl"}`,
		`"name": "subprocess", "profileRef": "file://mux-subprocess-profile.json"`,
		`"isolation": {"mode": "isolated_process"}`,
	...
	for _, want := range []string{
		...
		`json:"otelCompatible,omitempty"`,
		`json:"version,omitempty"`,
		`json:"schemaVersion,omitempty"`,
		`json:"sink,omitempty"`,
		`json:"profile,omitempty"`,
		`json:"profileRef,omitempty"`,
		`json:"fileSecretRoot,omitempty"`,
		`json:"operatorHistory,omitempty"`,
		"type RPCMuxOperatorHistoryConfig struct",
		`json:"maxActions,omitempty"`,
		...
		`json:"breakerCooldown,omitempty"`,
		...
		"c.Log.OTelCompatible.OperatorHistoryStore()",
		...
		"rpc.NewRPCMuxDiagnosisFileSecretResolver(c.FileSecretRoot, 0)",
		"rpc.NewRPCMuxDiagnosisOperatorHistoryFileStoreWithConfig(",
		"rpc.RPCMuxDiagnosisOperatorHistoryFileStoreConfig{",
		"rpc.NewRPCMuxDiagnosisLayeredSecretResolver",
	...
	for _, want := range []string{
		...
		"c.RPC.Mux.Log.OTelCompatible.NewSinkSet()",
		"c.RPC.Mux.ServerOptionsWithSinkSet(muxSinkSet)",
	...
	rpcTestData, err := os.ReadFile(filepath.Join(rpcDir, "internal", "api", "grpc", "v1", "greeter", "greeter_client_test.go"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
