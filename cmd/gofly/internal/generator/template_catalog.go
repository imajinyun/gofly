...
func builtInProjectTemplates() []ProjectTemplate {
	return []ProjectTemplate{
		...
		},
		{
			...
			Inputs:             commonTemplateInputs(),
			Files:              []string{"go.mod", "cmd/<name>/main.go", "etc/<name>.json", "etc/<name>-profile-candidate.json", "internal/config/config.go", "internal/mq/broker.go", "internal/observability/observability.go", "internal/routes/routes.go", "internal/svc/service.go"},
			Verify:             []string{"gofmt", "go mod tidy", "go test ./...", "gofly gateway profile validate --config etc/<name>.json --candidate etc/<name>-profile-candidate.json --json"},
			VerifyE2EValidated: true,
			RiskLevel:          "medium",
