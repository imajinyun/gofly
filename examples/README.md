...
| `resilience` | Limiter, breaker and retry behavior under mixed outcomes | `go run ./examples/resilience` | none | `go run -C examples/resilience . --json` | `gofly.resilience_drill.v1` JSON with rate-limit rejection, retry, breaker-open and recovery evidence |
...
Run the plugin ecosystem matrix:
