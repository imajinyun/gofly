...
func (s *ExperimentalMuxServer) DiagnosisSnapshot() RPCMuxTransportDiagnosis {
	...
	for _, adapter := range adapters {
		...
		diag.Transport.FramesIn += snapshot.Transport.FramesIn
		diag.Transport.FramesOut += snapshot.Transport.FramesOut
		diag.Transport.DataFramesIn += snapshot.Transport.DataFramesIn
		diag.Transport.DataFramesOut += snapshot.Transport.DataFramesOut
		diag.Transport.FragmentFramesIn += snapshot.Transport.FragmentFramesIn
		diag.Transport.FragmentFramesOut += snapshot.Transport.FragmentFramesOut
		...
		diag.Transport.CanceledStreams += snapshot.Transport.CanceledStreams
		diag.Transport.GoAwayFramesOut += snapshot.Transport.GoAwayFramesOut
		diag.Transport.BytesIn += snapshot.Transport.BytesIn
		diag.Transport.BytesOut += snapshot.Transport.BytesOut
		diag.Transport.ReceiveQueueSize = snapshot.Transport.ReceiveQueueSize
		diag.Transport.ConnectionWindow = snapshot.Transport.ConnectionWindow
		diag.Transport.MaxFrameBytes = snapshot.Transport.MaxFrameBytes
		diag.Transport.MaxMessageBytes = snapshot.Transport.MaxMessageBytes
		diag.Transport.MaxStreams = snapshot.Transport.MaxStreams
		diag.FlowControl.ConnectionWindow = snapshot.Transport.ConnectionWindow
		diag.FlowControl.ReceiveQueueSize = snapshot.Transport.ReceiveQueueSize
		diag.FlowControl.ConnectionCreditWaits += snapshot.Transport.ConnectionCreditWaits
		diag.FlowControl.StreamCreditWaits += snapshot.Transport.CreditWaits
		diag.FlowControl.CreditWaitTimeouts += snapshot.Transport.CreditWaitTimeouts
		diag.FlowControl.WriteTimeouts += snapshot.Transport.WriteTimeouts
		diag.FlowControl.ConnectionWindowExhausted += snapshot.Transport.ConnectionWindowExhausted
		diag.FlowControl.WindowFramesIn += snapshot.Transport.WindowFramesIn
		diag.FlowControl.WindowFramesOut += snapshot.Transport.WindowFramesOut
		diag.FlowControl.ConnectionWindowIn += snapshot.Transport.ConnectionWindowFramesIn
		diag.FlowControl.ConnectionWindowOut += snapshot.Transport.ConnectionWindowFramesOut
		diag.FlowControl.BackpressureEvents += snapshot.Transport.BackpressureEvents
